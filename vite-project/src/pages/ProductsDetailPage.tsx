...
const ProductsDetailPage = () => {
  ...
  const { allData, getProducts } = useProducts();
  ...
  useEffect(() => {
    ...
    if (allData?.length !== 0) {
      setTimeout(() => {
        const findSelectedItem = allData?.find((item) => item.id == id);
    ...
    <div>
      ...
      {loading ? (
        ...
        <>
              ...
              <div>
                ...
                <div className="flex items-center gap-x-4 mt-3">
                  ...
                  <Button
                    ...
                    onClick={() => handleAddToCart(data?.id as number)}
                  />
                </div>
                ...
                <h3 className="text-lg font-semibold mt-2">
                  Warranty Information: {data?.warrantyInformation}
                </h3>
              </div>
            </div>
          </div>
          <div>
            <hr className="mt-36" />
            <SimilarProducts selectedItem={data} />
          </div>
        </>
      )}
    </div>
